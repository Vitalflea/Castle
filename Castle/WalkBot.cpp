...
void inline Reset()
{
	if (reachedEnd || Settings::WalkBot::forceReset || hasDied) {
		pCvar->ConsoleColorPrintf(Color(225, 225, 10), "--Reset Rally--\n");
		rally = 0;
		reachedEnd = false;
...
bool DoRally(const std::vector<Vector> points, CUserCmd *cmd) // return true if rally is completed.
{
	...
	if ((std::abs(localPlayer->GetEyePosition().x - points[rally].x) < 0.6f) &&
		...
		if (rally == points.size() - 1)
		{
			if (!reachedEnd)
			{
				//pCvar->ConsoleColorPrintf(Color(225, 225, 10), "--Reached Rally--\n");
		...
		//pCvar->ConsoleColorPrintf(Color(225, 225, 10), "--Reached Rally--\n");
		rally++;
		//pCvar->ConsoleColorPrintf(Color(225, 225, 10), "Moving To Rally\n");
	}
...
void Walkbot::CreateMove(CUserCmd *cmd)
{
	...
	if ((*pGameRules)->IsFreezeTime())
	{
		...
		{
			pEngine->ExecuteClientCmd("autobuy");
			hasAutobought = true;
		}
	}
	...
	if (!localPlayer->GetAlive())
	{
		...
		{
			pEngine->ExecuteClientCmd("teammenu"); // this will trigger the auto select timer
			return;
		}
		else
		{
